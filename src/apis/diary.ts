...
export const getFindOne = async (diaryId) => {
  try {
    const data = await get(`/api/v2/member/diary/findOne/${diaryId}`);
    return data;
  } catch (error) {
    throw new Error("데이터 불러오기에 실패하였습니다.");
  }
...
export const getFindAll = async () => {
  try {
    const data = await get(
      `/api/v2/member/diary/findAllPageable?page=0&size=100`
    );
    return data;
  } catch (error) {
    throw new Error("데이터 불러오기에 실패하였습니다.");
...
export const getFindAllByEmotion = async ({ emotion }) => {
  try {
    const url = `/api/v2/member/diary/findAllByEmotionWithPageable?diaryEmotion=${emotion}&page=0&size=100&sort=string`;
...
export const getFindAllByFilter = async (searchParams) => {
  try {
    const url = `/api/v2/member/diary/findAllByFilter`;
    const params = {
